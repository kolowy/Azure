...
function setPresence(bot) {
  ...
  if (day === 25 && mounth === 12) {
    ...
    bot.user.setPresence({ game: { name: `.help | Use by ${getCountUsers()} users.`, type: 'PLAYING' }, status: 'online' }) // Change to "do not distibute" for Christmas.
      .then(() => console.log('activity refresh'))
      .catch(console.log)
  }
}
...
bot.on('message', message => {
  ...
  if (message.content.startsWith(prefix + "clear")) {
    ...
    if (args > 100) {
      message.channel.send("100 is the maximum.")
    } else {
      message.delete().catch(O_o => { });
